...
type (
	...
	FileInfo struct {
		...
		Uploader    string    `db:"uploader"`     // 上传者用户ID
...
func (m *defaultFileInfoModel) Insert(ctx context.Context, data *FileInfo) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.FileId, data.FileName, data.Bucket, data.ObjectName, data.Size, data.ContentType, data.Uploader, data.UploadTime, data.Status)
	}, fileInfoFileIdKey, fileInfoIdKey)
	return ret, err
}
...
func (m *defaultFileInfoModel) Update(ctx context.Context, newData *FileInfo) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.FileId, newData.FileName, newData.Bucket, newData.ObjectName, newData.Size, newData.ContentType, newData.Uploader, newData.UploadTime, newData.Status, newData.Id)
	}, fileInfoFileIdKey, fileInfoIdKey)
	return err
}
